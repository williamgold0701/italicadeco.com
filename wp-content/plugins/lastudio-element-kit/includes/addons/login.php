...
class LaStudioKit_Login extends LaStudioKit_Base {
    ...
    protected function render() {
		...
		$login_form = str_replace(['id="user_login"', 'id="user_pass"'], ['id="user_login" placeholder="'.$settings['label_username'].'"', 'id="user_pass" placeholder="'.$settings['label_password'].'"'], $login_form);
	...
	public function login_form_middle( $content ){
		...
		$lost_password_link = $this->get_settings_for_display('lost_password_link');
		if(filter_var($lost_password_link, FILTER_VALIDATE_BOOLEAN)){
			$content .= sprintf('<a class="lakit-login-lost-password-link" href="%1$s">%2$s</a>', esc_url(wp_lostpassword_url()), $this->get_settings_for_display('lost_password_link_text'));
		}
		return $content;
	}
